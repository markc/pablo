document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('click', function (event) {
        if (isMobile) {
            ...
            const isClickInsideRightSidebar = rightSidebar.contains(
                event.target
            );
            const isClickOnLeftToggle =
                event.target.closest('#leftSidebarToggle');
            const isClickOnRightToggle = event.target.closest(
                '#rightSidebarToggle'
            );
    ...
    async function loadContent(url) {
        try {
            ...
            setTimeout(() => {
                ...
                Array.from(
                    contentSection.getElementsByTagName('script')
                ).forEach((script) => {
